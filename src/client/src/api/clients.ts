...
export const sessionsApi = {
  ...
  startSession: (cwd: string, machineId = "local", startupToken?: string, backend?: SessionBackend) => request(`${machinePrefix(machineId)}/sessions`, parseSessionInfo, { method: "POST", body: JSON.stringify({ cwd, ...(startupToken === undefined ? {} : { startupToken }), ...(backend === undefined ? {} : { backend }) }) }),
