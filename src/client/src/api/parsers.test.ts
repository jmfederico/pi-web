...
describe("API parsers", () => {
  ...
  it("parses PI WEB config responses", () => {
    expect(parsePiWebConfigResponse({
      ...
      exists: true,
      config: { host: "0.0.0.0", port: 8504, allowedHosts: ["example.local"], shortcuts: { "core:view.chat": "mod+1", "core:session.stop": null }, breadcrumbMode: "compact", pinnedWorkspaceTools: ["core:workspace.files"], plugins: { info: { enabled: false, settings: { compact: true } } }, pathAccess: { allowedPaths: ["/tmp"] }, uploads: { defaultFolder: "manual/uploads" }, maxUploadBytes: 1234, agent: { command: "agent-lab", dir: "~/agent-profiles/lab" } },
