...
describe("federated route contract", () => {
  ...
  it("covers machine-scoped client HTTP calls with remote proxy routes", async () => {
    ...
    await Promise.all([
      ...
      ignoreParseFailure(sessionsApi.authProviders({ mode: "login", authType: "oauth", machineId, target: session })),
      ignoreParseFailure(sessionsApi.saveApiKey("openai", "key", machineId, "provider-ref")),
      ignoreParseFailure(sessionsApi.startInteractiveApiKeyLogin("amazon-bedrock", machineId, "provider-ref")),
      ignoreParseFailure(sessionsApi.logoutProvider("openai", machineId)),
      ignoreParseFailure(sessionsApi.startOAuthLogin("openai", machineId, "provider-ref")),
