...
function parsePiWebConfigValues(value: unknown): PiWebConfigValues {
  ...
  return {
    ...
    ...optionalField("breadcrumbMode", optionalBreadcrumbMode(record["breadcrumbMode"])),
    ...optionalField("pinnedWorkspaceTools", optionalPinnedWorkspaceTools(record["pinnedWorkspaceTools"])),
...
function optionalBreadcrumbMode(value: unknown): PiWebBreadcrumbMode | undefined {
  if (value === undefined) return undefined;
  if (value === "expanded" || value === "compact") return value;
  throw new Error("Invalid PI WEB breadcrumbMode field");
}

function optionalPinnedWorkspaceTools(value: unknown): string[] | undefined {
  if (value === undefined) return undefined;
  if (!Array.isArray(value)) throw new Error("Invalid PI WEB pinnedWorkspaceTools field");
  const ids: string[] = [];
  for (const entry of value) {
    if (typeof entry !== "string" || !workspaceToolIdPattern.test(entry)) throw new Error("Invalid PI WEB pinnedWorkspaceTools field");
