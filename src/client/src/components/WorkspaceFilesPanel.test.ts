...
function workspacePanelContext(patch: Partial<WorkspacePanelContext> = {}): WorkspacePanelContext {
  ...
  return {
    ...
    backend: patch.backend ?? { request: vi.fn<NonNullable<WorkspacePanelContext["backend"]>["request"]>(() => Promise.resolve(null)), requestBinary: vi.fn<NonNullable<WorkspacePanelContext["backend"]>["requestBinary"]>(() => Promise.resolve(null)) },
