...
export class ToolExecutionView extends LitElement {
  ...
  /** Renders without the card chrome and title, for hosts that already label the execution. */
  ...
  override render() {
    ...
    return html`
      <section class=${`tool-card ${execution.status}`}>
        ${this.renderHeader(execution, diffStats, target)}
  ...
  private renderHeader(execution: ToolExecutionPart, diffStats: { added: number; removed: number } | undefined, target: ToolTarget | undefined) {
    const editCount = editCountLabel(execution);
    const meta = html`
      <div class="tool-meta">
        ${editCount === undefined ? null : html`<span>${editCount}</span>`}
        ...
        ${this.bare ? null : html`<span class="status-label">${statusLabel(execution.status)}</span>`}
      </div>
    `;
    if (this.bare) {
      if (editCount === undefined && diffStats === undefined) return null;
      return html`<div class="tool-header">${meta}</div>`;
    }
    return html`
  ...
  private renderTextBody(text: string | undefined, open: boolean, target: ToolTarget | undefined) {
    ...
    if (this.bare) return html`
      <div class="text-body">
        ${this.renderTextBodyContent(text, target)}
      </div>
    `;
    return html`
  ...
  private renderTextBodyContent(text: string | undefined, target: ToolTarget | undefined) {
    return html`
  ...
  private renderDiffBody(diff: string, label: string, target: ToolTarget | undefined) {
    ...
    const body = html`
      ${this.renderExpandedTarget(target)}
      ${this.renderDiffToolbar(diff, truncated, visibleLines.length, lines.length)}
      ...
      ` : null}
    `;
    if (this.bare) return html`<div class="diff-details">${body}</div>`;
    return html`
  ...
  private renderDiffToolbar(diff: string, truncated: boolean, visibleCount: number, totalCount: number) {
    return html`
      <div class="diff-toolbar">
          <span>${truncated ? `Showing ${String(visibleCount)} of ${String(totalCount)} lines` : "Full diff"}</span>
  ...
  static override styles = css`
    ...
    .tool-card { display: grid; gap: 8px; width: 100%; max-width: 100%; min-width: 0; box-sizing: border-box; overflow: hidden; border: 1px solid var(--pi-border); border-radius: 10px; background: var(--pi-bg); padding: 9px; color: var(--pi-text); }
    ...
    .error-text { margin: 0; border: 1px solid var(--pi-danger); border-radius: 9px; background: color-mix(in srgb, var(--pi-danger) 10%, var(--pi-bg)); color: var(--pi-danger); padding: 8px; white-space: pre-wrap; overflow-wrap: anywhere; font: 12px ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; }
    ...
    button { border: 1px solid var(--pi-border); border-radius: 8px; background: var(--pi-surface); color: var(--pi-text); padding: 3px 7px; font: 12px system-ui, sans-serif; cursor: pointer; }
    button:hover, button:focus { border-color: var(--pi-accent); }
    .diff { box-sizing: border-box; width: 100%; max-width: 100%; min-width: 0; margin: 0; overflow-x: auto; overflow-y: hidden; overscroll-behavior-x: contain; border: 1px solid var(--pi-border-muted); border-radius: 9px; background: var(--pi-bg); padding: 8px 0; color: var(--pi-muted); font: 12px ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; line-height: 1.45; }
