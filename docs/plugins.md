...
After installing, removing, or updating a Pi package, type `/reload` in each idle, message-bearing PI WEB session on the target machine to refresh Pi runtime resources such as extensions, skills, prompt templates, themes, and context/system prompt files as supported by Pi. PI WEB intentionally rejects resource reload in a pristine session or at a message-empty session-tree root because rebuilding there would alter Pi's session tree. Close and start a new pristine session to load current resources; from a tree root, navigate to a message-bearing entry, run `/reload`, then navigate back. Reload the browser page separately for newly discovered or changed PI WEB browser plugins. A routine session daemon restart is not required.
